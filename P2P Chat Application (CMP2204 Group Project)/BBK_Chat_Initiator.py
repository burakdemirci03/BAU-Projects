...
def initiate_chat():  # Function to initiate a chat session with another user
    ...
    selected_user_address = next((address for address, info in peers.items() if info['username'] == selected_user), None)

    if selected_user_address is None:
        print("Error: Could not find the selected user.")
        return
    ...
    if secure:  # If the chat is secure
        ...
        try:
            sock_tcp.connect((selected_user_address, 6001))
